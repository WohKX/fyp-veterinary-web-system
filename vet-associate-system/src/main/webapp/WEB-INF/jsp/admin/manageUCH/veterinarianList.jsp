...
<title>Veterinarian Listing</title>
								...
								<li class="nav-item">
									<div class="row">
										<button type="submit" class="btn btn-black px-4"
											...
											value="ManageVeterinarian" disabled>Veterinarian</button>
									</div>
								</li>
								<li class="nav-item">
									<div class="row">
					...
					<div class="container">
						<c:choose>
							<c:when test="${veterinarianList.size() > 0 }">
								<form action="/manageUCH/admin/viewVet" method="post">
										...
										<tbody>
											<c:forEach items="${veterinarianList}" var="veterinarian">
												<tr>
													...
													<td><c:out value="${veterinarian.user.accStatus}" /></td>
													<td><button class="btn btn-outline-info btn-sm"
															type="submit" name="id" value="${veterinarian.id}">View</button></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
							...
							<c:otherwise>
								<div class="container justify-content-center fs-3">
									<span class="badge rounded-pill bg-info text-dark">There
										are no veterinarian available currently.</span>
								</div>
							</c:otherwise>
						</c:choose>
					</div>
